# 🛡️ Jan Sahay — Citizen AI Voice Assistant Portal
...
- 🏛️ **Government Schemes Directory**: Discover central and state welfare schemes (like PM-KISAN, PMJDY, PMSBY, APY, Mudra loans), check eligibility, and understand documentation.
...
* **Location**: `backend/caller_data.db` (initialized automatically via [db.py](file:///f:/agent%20ser/murf-livekit-starter/backend/src/db.py))
...
* **Location**: Implemented within the rules engine in the `check_scheme_eligibility` tool in [agent.py](file:///f:/agent%20ser/murf-livekit-starter/backend/src/agent.py).
* **Usage**: Provides instant, rule-based eligibility verification, premium schedules, interest rates, and required document checklists for five major national financial inclusion schemes:
